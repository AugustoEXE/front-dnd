<script lang="ts">
	...
	function getData() {
		if (browser) {
			...
			axi.get('/creature-types').then((res) => {
				types = res.data;
				loading = false;
			});
		}
	...
	function deleteItem(id: number) {
		if (browser) {
			...
			axi.delete(`/creature-types/${id}`).then(() => {
	...
	function handleSubmmit() {
		if (browser) {
			...
			axios
				.post('http://localhost:8000/creature-types', formData, {
					headers: {
...
<div class="m-auto w-5/6 text-center h-screen text-neutral-300 mb-5">
	<h1 class="text-5xl mb-10 mt-5 font-ancient">Listagem de Tipos de Criaturas</h1>
	{#if loading}
	<Loading/>
	{:else}
		<div class="flex items-center">
			...
			<dialog id="my_modal_4" class="modal bg-transparent backdrop-blur transition duration-150">
				<div class="modal-box w-11/12 max-w-5xl bg-zinc-900 rounded-xl">
					<h3 class="font-bold text-lg">Cadastre um Tipo de criatura!</h3>
			...
			<p>Adicionar novo Tipo de criatura</p>
		</div>
