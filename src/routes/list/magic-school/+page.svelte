<script lang="ts">
    ...
    let schools: Array<any> = [], loading=true;
    onMount(()=>{
        getData()
    })
    auth()
	...
	function getData() {
		if (browser) {
			...
			axi.get('/magic-schools').then((res) => {schools = res.data; loading=false});
	...
	function deleteItem(id: number) {
		if (browser) {
			...
			axi
				.delete(`/magic-schools/${id}`)
				.then(() => {schools = schools.filter((item) => item.id !== id); loading=false});
		}
	}
	...
	function handleSubmmit() {
		if (browser) {
			...
			axios
				.post('http://localhost:8000/magic-schools', formData, {
					headers: {
...
<div class="m-auto w-5/6 text-center h-screen text-neutral-300 mb-5">
	<h1 class="text-5xl mb-10 mt-5 font-ancient">Listagem de Escolas de Magia</h1>
{#if loading}
	<Loading/>
{:else}
	 <!-- else content here -->
	<div class="flex items-center">
		...
		<p>Adicionar nova escola de magia</p>
	</div>
